...
var phishlets_dir = flag.String("p", "", "BWG Phishlets directory path")
...
func showAd() {
	...
	message := fmt.Sprintf("%s: %s %s", lred.Sprint("Private Evilginx edition"), lyellow.Sprint("Evilginx Support"), white.Sprint("(@evilginxsupport)"))
	log.Info("%s", message)
}
...
func main() {
	...
	log.Info("BWG Phishlets : %s", phishlets_path)
